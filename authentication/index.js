...
const User = require("../models/UserSchema/index")
            ...
            const cmp = await bcrypt.compare(password,user.password)
                if(user){
                    ...
                    if(user && cmp){
                        ...
                        const accessToken = jwt.sign({user:{username:user.username, email:user.email,id:user._id,stripeCustomer:stripeCustomer.id}},tokenSecret,{expiresIn:"3m"})
...
module.exports = {login,signup,forgotPassword,resetLink,resetPassword,localVariables,confirmOTP,logout}